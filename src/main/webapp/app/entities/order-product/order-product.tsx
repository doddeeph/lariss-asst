...
export const OrderProduct = () => {
  ...
  const totalItems = useAppSelector(state => state.orderProduct.totalItems);
  ...
  return (
    <div>
      <h2 id="order-product-heading" data-cy="OrderProductHeading">
        <Translate contentKey="larissAsstApp.orderProduct.home.title">Order Products</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="larissAsstApp.orderProduct.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/order-product/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="larissAsstApp.orderProduct.home.createLabel">Create new Order Product</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderProductList && orderProductList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="larissAsstApp.orderProduct.id">ID</Translate>{' '}
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {orderProduct.productDetails ? (
                      <Link to={`/product-details/${orderProduct.productDetails.id}`}>{orderProduct.productDetails.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/order-product/${orderProduct.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/order-product/${orderProduct.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="larissAsstApp.orderProduct.home.notFound">No Order Products found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={orderProductList && orderProductList.length > 0 ? '' : 'd-none'}>
