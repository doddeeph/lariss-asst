  ...
  return (
    <div>
      <h2 id="payment-heading" data-cy="PaymentHeading">
        <Translate contentKey="larissAsstApp.payment.home.title">Payments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="larissAsstApp.payment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/payment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="larissAsstApp.payment.home.createLabel">Create new Payment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {paymentList && paymentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="larissAsstApp.payment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('cvv')}>
                  <Translate contentKey="larissAsstApp.payment.cvv">Cvv</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('cvv')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {paymentList.map((payment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`larissAsstApp.PaymentMethod.${payment.paymentMethod}`} />
                  </td>
                  <td>{payment.cardNumber}</td>
                  <td>
                    {payment.expirationDate ? <TextFormat type="date" value={payment.expirationDate} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{payment.cvv}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="larissAsstApp.payment.home.notFound">No Payments found</Translate>
            </div>
          )
        )}
      </div>
